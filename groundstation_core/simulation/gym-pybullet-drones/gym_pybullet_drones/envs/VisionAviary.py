...
class VisionAviary(BaseAviary):
    ...
    def _computeObs(self):
        ...
        for i in range(self.NUM_DRONES):
            if self.step_counter%self.IMG_CAPTURE_FREQ == 0:
                self.rgb[i], self.dep[i], self.seg[i] = self._getDroneImages(i,segmentation=True)
    ...
    def _computeInfo(self):
        ...
        for nth_drone in range(self.NUM_DRONES):
            ...
            dict[str(nth_drone)] = {"ViewMatrix": DRONE_CAM_VIEW,
                                    "ProjectionMatrix": DRONE_CAM_PRO}
        return  dict #### Calculated by the Deep Thought supercomputer in 7.5M years
