...
class BaseAviary(gym.Env):
    ...
    def _housekeeping(self):
        ...
        p.changeVisualShape(self.PLANE_ID, -1, rgbaColor=[1, 1, 1, 1], specularColor=[0, 0, 0])
    ...
    def _getDroneImages(self,
        ...
        # rgb = np.zeros(shape = (h, w, 4))#np.reshape(rgb, (h, w, 4))
        # dep = np.zeros(shape = (h, w))#np.reshape(dep, (h, w))
        # seg = np.zeros(shape = (h, w))#np.reshape(seg, (h, w))
        return rgb, dep, seg
    ...
    def _addObstacles(self):
        ...
        """
        # p.loadURDF("obs000.urdf",
        #            [1, 1, 1],
        #            physicsClientId=self.CLIENT
        #            )
        # p.loadURDF("obs002.urdf",
        #            [1, 1, 1],
        #            physicsClientId=self.CLIENT
        #            )
