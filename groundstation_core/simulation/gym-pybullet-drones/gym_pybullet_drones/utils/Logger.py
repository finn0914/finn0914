...
class Logger(object):
    ...
    def save(self):
        ...
        """
        min_length = min(len(self.timestamps), len(self.states), len(self.controls))

        # 截断数组
        timestamps_truncated = self.timestamps[:min_length]
        states_truncated = self.states[:min_length]
        controls_truncated = self.controls[:min_length]

        with open(os.path.join(self.OUTPUT_FOLDER, "save-flight-"+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")+".npy"), 'wb') as out_file:
            np.savez(out_file, timestamps=timestamps_truncated, states=states_truncated, controls=controls_truncated)
    ...
    def save_as_csv(self,
        ...
        for i in range(self.NUM_DRONES):     
            t_adjusted = t[:len(self.states[i, 0, :])]       
            with open(csv_dir+"/x"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 0, :]])), delimiter=",")
            with open(csv_dir+"/y"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 1, :]])), delimiter=",")
            with open(csv_dir+"/z"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 2, :]])), delimiter=",")
            ####
            with open(csv_dir+"/r"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 6, :]])), delimiter=",")
            with open(csv_dir+"/p"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 7, :]])), delimiter=",")
            with open(csv_dir+"/ya"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 8, :]])), delimiter=",")
            ####
            with open(csv_dir+"/rr"+str(i)+".csv", 'wb') as out_file:
                rdot = np.hstack([0, (self.states[i, 6, 1:] - self.states[i, 6, 0:-1]) * self.LOGGING_FREQ_HZ ])
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, rdot])), delimiter=",")
            with open(csv_dir+"/pr"+str(i)+".csv", 'wb') as out_file:
                pdot = np.hstack([0, (self.states[i, 7, 1:] - self.states[i, 7, 0:-1]) * self.LOGGING_FREQ_HZ ])
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, pdot])), delimiter=",")
            with open(csv_dir+"/yar"+str(i)+".csv", 'wb') as out_file:
                ...
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, ydot])), delimiter=",")
            ###
            with open(csv_dir+"/vx"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 3, :]])), delimiter=",")
            with open(csv_dir+"/vy"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 4, :]])), delimiter=",")
            with open(csv_dir+"/vz"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 5, :]])), delimiter=",")
            ####
            with open(csv_dir+"/wx"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 9, :]])), delimiter=",")
            with open(csv_dir+"/wy"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 10, :]])), delimiter=",")
            with open(csv_dir+"/wz"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 11, :]])), delimiter=",")
            ####
            with open(csv_dir+"/rpm0-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 12, :]])), delimiter=",")
            with open(csv_dir+"/rpm1-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 13, :]])), delimiter=",")
            with open(csv_dir+"/rpm2-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 14, :]])), delimiter=",")
            with open(csv_dir+"/rpm3-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, self.states[i, 15, :]])), delimiter=",")
            ####
            with open(csv_dir+"/pwm0-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, (self.states[i, 12, :] - 4070.3) / 0.2685])), delimiter=",")
            with open(csv_dir+"/pwm1-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, (self.states[i, 13, :] - 4070.3) / 0.2685])), delimiter=",")
            with open(csv_dir+"/pwm2-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, (self.states[i, 14, :] - 4070.3) / 0.2685])), delimiter=",")
            with open(csv_dir+"/pwm3-"+str(i)+".csv", 'wb') as out_file:
                np.savetxt(out_file, np.transpose(np.vstack([t_adjusted, (self.states[i, 15, :] - 4070.3) / 0.2685])), delimiter=",")
    ...
    def plot(self, pwm=False):
        ...
        t = np.arange(0, self.timestamps.shape[1]/self.LOGGING_FREQ_HZ, 1/self.LOGGING_FREQ_HZ)
        t_adjusted = t[:len(self.states[0, 0, :])]  
        ...
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]       
            axs[row, col].plot(t_adjusted, self.states[j, 0, :], label="drone_"+str(j))
        ...
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]  
            axs[row, col].plot(t_adjusted, self.states[j, 1, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        ...
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]  
            axs[row, col].plot(t_adjusted, self.states[j, 2, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        ...
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]  
            axs[row, col].plot(t_adjusted, self.states[j, 6, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('r (rad)')
        row = 4
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]  
            axs[row, col].plot(t_adjusted, self.states[j, 7, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('p (rad)')
        row = 5
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]  
            axs[row, col].plot(t_adjusted, self.states[j, 8, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        ...
        for j in range(self.NUM_DRONES):
            t_adjusted = t[:len(self.states[j, 0, :])]  
            axs[row, col].plot(t_adjusted, self.states[j, 9, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('wx')
        row = 7
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 10, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('wy')
        row = 8
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 11, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('wz')

        #### Time ##################################################
        row = 9
        axs[row, col].plot(t_adjusted, t_adjusted, label="time")
        axs[row, col].set_xlabel('time')
        ...
        row = 0
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 3, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('vx (m/s)')
        row = 1
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 4, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('vy (m/s)')
        row = 2
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 5, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        ...
        row = 6
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 12, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        if pwm:
        ...
        row = 7
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 13, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        if pwm:
        ...
        row = 8
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 14, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        if pwm:
        ...
        row = 9
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t_adjusted, self.states[j, 15, :], label="drone_"+str(j))
        axs[row, col].set_xlabel('time')
        if pwm:
